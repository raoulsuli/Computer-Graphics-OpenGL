...
void Laborator2::Init()
{
	...
	// Create tetrahedron
	{
		...
		meshes["tetrahedron"]->InitFromData(verticesT, indicesT);

		// Create a new mesh from buffer data
		Mesh* tetrahedron = CreateMesh("tetrahedron", verticesT, indicesT);
	}
	...
	// Create square
	{
		...
		meshes["square"]->InitFromData(verticesS, indicesS);

		// Create a new mesh from buffer data
		Mesh* square = CreateMesh("square", verticesS, indicesS);
	}
}
...
void Laborator2::Update(float deltaTimeSeconds)
{
	...
	RenderMesh(meshes["tetrahedron"], shaders["VertexColor"], glm::vec3(1, 0.5f, -0.5f), glm::vec3(0.5f));

	RenderMesh(meshes["square"], shaders["VertexColor"], glm::vec3(-0.5f, 0.5f, 1), glm::vec3(0.5f));
