...
void Laborator4::Update(float deltaTimeSeconds)
{
	...
	modelMatrix = glm::mat4(1);
	modelMatrix *= Transform3D::Translate(0, 3, -5);
	...
	modelMatrix *= Transform3D::RotateOY(sunAngle * 2);
	modelMatrix *= Transform3D::Translate(0, 0, 3.5f);
	RenderMesh(meshes["sphere"], shaders["VertexNormal"], modelMatrix);

	// moon
	modelMatrix *= Transform3D::RotateOY(sunAngle * 3);
	modelMatrix *= Transform3D::Translate(0, 0, 1.25f);
	RenderMesh(meshes["sphere"], shaders["VertexNormal"], modelMatrix);
}
